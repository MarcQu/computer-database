		...
		fin : while(true) {
			...
			switch (table) {
				case "company" :
					...
					System.out.println(" - aide : r�afficher les commandes");
					System.out.println(" - retour : retour au menu pr�c�dent");
					System.out.print("-> ");
				...
				case "computer" :
					...
					System.out.println(" - createComputer : ajoute un ordinateur dans la base de donn�es");
					System.out.println(" - updateComputer : met � jour les informations d'un ordinateur");
					System.out.println(" - deleteComputer : supprime un ordinateur de la base de donn�es");
					System.out.println(" - aide : r�afficher les commandes");
					System.out.println(" - retour : retour au menu pr�c�dent");
					System.out.print("-> ");
		...
		retour : while(true) {
			...
			switch (command) {
				case "listCompanies" :
					...
					System.out.println("Combien de r�sultats?");	
		...
		retour : while(true) {
			...
			switch (command) {
				...
				case "createComputer" :
					...
					while("".equals(name)) {
						System.out.println("Le nom ne peut pas �tre vide");
					...
					if (!"".equals(introduced) && !"".equals(discontinued)) {
						while(!discontinued.matches("^\\d{4}-(0\\d|1[0-2])-(0\\d|[1-2]\\d|3[0-1])\\s[0-5]\\d:[0-5]\\d:[0-5]\\d$") || Timestamp.valueOf(introduced).after(Timestamp.valueOf(discontinued))) {
							System.out.println("La date d'interruption doit �tre post�rieure � la date d'introduction et de la forme YYYY-MM-DD HH:MM:SS");
					...
					break;
				case "updateComputer" :
					System.out.println("Entrer l'id de l'ordinateur � modifier : ");
					id = sc.nextLine();
					//Boucle qui vérifie que l'id est un entier et non vide
					while("".equals(id) || !id.matches("^\\d+$")) {
						System.out.println("L'id doit �tre un entier et non vide");
					...
					companyId = sc.nextLine();
					while(!"".equals(companyId) && !companyId.matches("^\\d+$")) {
						System.out.println("L'id companie doit �tre un entier ou vide");
					...
					break;
				case "deleteComputer" :
					System.out.println("Entrer l'id de la companie � supprimer : ");
					id = sc.nextLine();
					//Boucle qui vérifie que l'id est un entier ou vide
					while(!"".equals(id) && !id.matches("^\\d+$")) {
						System.out.println("L'id doit �tre un entier ou vide");
						System.out.println("Entrer un nouvelle id d'ordinateur : ");
						id = sc.nextLine();
					}
					if (!id.equals("")) {
					...
					System.out.println("Entrer le nom de la companie � supprimer : ");
					name = sc.nextLine();
					if (!name.equals("")) {
						champs.add("name");
					}
					System.out.println("Entrer la date d'introduction de la companie � supprimer : (YYYY-MM-DD HH:MM:SS)");
					introduced = sc.nextLine();
					//Boucle qui vérifie que le format de la date est correct ou vide
					while(!"".equals(introduced) && !introduced.matches("^\\d{4}-(0\\d|1[0-2])-(0\\d|[1-2]\\d|3[0-1])\\s[0-5]\\d:[0-5]\\d:[0-5]\\d$")) {
					...
					System.out.println("Entrer la date d'interruption de la companie � supprimer : (YYYY-MM-DD HH:MM:SS)");
					discontinued = sc.nextLine();
					//Boucle qui vérifie que le format de la date est correct ou vide
					while(!"".equals(discontinued) && !discontinued.matches("^\\d{4}-(0\\d|1[0-2])-(0\\d|[1-2]\\d|3[0-1])\\s[0-5]\\d:[0-5]\\d:[0-5]\\d$")) {
						System.out.println("Entrer une date valide selon le format YYYY-MM-DD HH:MM:SS ou laisser le champ vide");
					...
					System.out.println("Entrer le company_id de la companie � supprimer : ");
					companyId = sc.nextLine();
